...
# IMU exercise
# Copyright (c) 2015-2026 Kjeld Jensen kjen@mmmi.sdu.dk kj@kjen.dk

"""
2022-09-12, Kjeld Jensen, Added info about pyopengl module
2026-09-03, Kjeld Jensen, Added a check for Wayland and a warning message
...
# Approximate calibration from the first 100 (stationary) samples of
# imu_razor_data_pitch_55deg.txt. Recalibrate for a different recording/IMU.
bias_gyro_x = 0.0685383208870121  # [rad/s]
bias_gyro_y = 0.04877208768877372  # [rad/s]
bias_gyro_z = 0.012250693671389763  # [rad/s]

# variances
gyroVar = 6.412885654323961e-05  # angular velocity variance [(rad/s)^2]
pitchVar = 6.075596165312929e-05  # accelerometer pitch variance [rad^2]

# Kalman filter start guess
estAngle = -pi / 4.0
estVar = (pi / 4.0) ** 2  # initial angle uncertainty [rad^2]

# Kalman filter housekeeping variables
gyroVarAcc = 0.0  # integrated gyro angle variance [rad^2]
...
if show3DLiveView and os.environ.get("XDG_SESSION_TYPE") == "wayland":
    print("Sorry, this demo requires an X11 session.")
    print("In Ubuntu log out, then when you select your name during login,")
    print("click the lower right toolbox. Here select 'Ubuntu on Xorg'.")
    print("Then it should work fine.")
    ...
    for line in f:
        ...
        kalman_gain = estVar / (estVar + pitchVar)
        estAngle += kalman_gain * (pitch - estAngle)
        estVar *= 1.0 - kalman_gain
        gyroVarAcc = 0.0  # all accumulated process noise was used above
